...
saberExperimentSetup () {
	...
	#
	# Window settings
	OPTS="$OPTS --window-type-of-first-stream  row --window-size-of-first-stream  1024 --window-slide-of-first-stream  1024"
	OPTS="$OPTS --window-type-of-second-stream row --window-size-of-second-stream 1024 --window-slide-of-second-stream 1024"
	# OPTS="$OPTS --window-type-of-first-stream  row --window-size-of-first-stream  64 --window-slide-of-first-stream  64"
	# OPTS="$OPTS --window-type-of-second-stream row --window-size-of-second-stream 64 --window-slide-of-second-stream 64"
...
saberExperimentRun () {
	...
	for mode in "cpu" "gpu" "hybrid"; do
		...
		*)
		...
		INTERMEDIATEBUFFERSIZE=$TASKSIZE 
		# INTERMEDIATEBUFFERSIZE=`echo "64 * 1024 * (32 + 32)" | bc`
		# 1024 = window size
		WINDOWSIZE=1024
		INTERMEDIATEBUFFERSIZE=`echo "$WINDOWSIZE * $TPI * (32 + 32)" | bc`

		echo "buffersize $BUFFERSIZE"
		echo "tasksize $TASKSIZE"
		echo "tpi $TPI"
		echo "ibs $INTERMEDIATEBUFFERSIZE"
		...
		"$SABER_HOME"/scripts/experiments/microbenchmarks/theta-join.sh $OPTS $modeoptions \
		--batch-size $TASKSIZE --intermediate-buffer-size $INTERMEDIATEBUFFERSIZE --circular-buffer-size $BUFFERSIZE --tuples-per-insert $TPI --execution-mode $mode
		
		errorcode=$?
		
		saberExperimentStoreResults "thetajoin" "$RESULTDIR" "$N-$key"
